...
entrez_id=mapIds(x=org.Rn.eg.db,keys=genes,keytype=genetype,column ="ENTREZID")
entrez_id=na.omit(entrez_id)
erich.go.ALL=enrichGO(gene=entrez_id,OrgDb=org.Rn.eg.db,keyType="ENTREZID",ont="ALL",pvalueCutoff=0.05,qvalueCutoff=0.05, readable=TRUE)
...
dev.off()
KEGG=enrichKEGG(gene=entrez_id,pvalueCutoff=0.05,qvalueCutoff=0.1,organism='rno')
