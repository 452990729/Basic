...
def MakePlot(pd_data, tp, title, xlabel, ylabel, size, out, rotation):
    ...
    if tp == 'stack':
        ...
        for i in range(pd_data.shape[1]):
            ...
            if i == 0:
                ...
                locals()['p'+str(i)] = plt.bar(x, pd_data.iloc[:, i], width, bottom=pd_data.iloc[:, :i].sum(axis=1))
        ax.set_xticks(x)
    elif tp == 'group':
    ...
    ax.set_xticklabels(list(pd_data.index), rotation=rotation)
    ax.set_title(title)
...
def main():
    ...
    MakePlot(pd_data, argv['type'], argv['title'], argv['xlabel'], argv['ylabel'], argv['size'], argv['out'], argv['rotation'])
